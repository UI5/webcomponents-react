...
to fit the frozen columns plus a usable scrollable area, and re-enables when it grows again. When the table
has no rows, sticky stays active (so the header doesn't jump when data loads) but the freeze-column line is
limited to the header — it doesn't extend into the empty body.

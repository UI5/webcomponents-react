...
<b>Experimental.</b> Opt in by passing the `useStickyColumns` plugin hook via `tableHooks`. Mark columns
with `sticky: 'start'` to freeze them at the inline-start (left in LTR, right in RTL) so they stay visible
...
<AnalyticalTable
  ...
  columns={[{ Header: 'Name', accessor: 'name', sticky: 'start' }, /* … */]}
  data={data}
/>;
```

The `sticky: 'start'` column option seeds the **initial** frozen set. At runtime the frozen set is state —
toggle it cheaply (no re-render of the row model) via the `tableInstance` prop ref:

```jsx
tableInstance.current.toggleStickyColumn('name'); // toggle one column
tableInstance.current.setStickyColumns(['name', 'age']); // replace the whole set
```

Internal columns (selection, highlight) auto-freeze alongside your sticky columns, and **grouped columns
automatically become sticky** and move to the start. Sticky auto-disables when the container is too narrow
to fit the frozen columns plus a usable scrollable area, and re-enables when it grows again. The frozen-set
config is **kept** across auto-disable (pins are not lost on resize); pass an `onAutoToggleSticky` callback
to `useStickyColumns` to react to that transition. When the table has no rows, sticky stays active (so the
header doesn't jump when data loads) but the freeze-column line is limited to the header — it doesn't extend
into the empty body.

<Canvas of={ComponentStories.StickyColumns} />

See the [`useStickyColumns` plugin page](?path=/docs/data-display-analyticaltable-plugin-hooks-usestickycolumns--docs) for the callbacks and a resizable-container example.

### Limitations

- **Not combinable with `renderRowSubComponent`** (row sub-components) or **`responsivePopIn`** (pop-in
  columns) — those layouts don't have a meaningful frozen-column region.
- When `responsivePopIn` collapses the table to a single visible column, there is no scrollable area, so
  sticky is inactive.
- Sticky mode makes the table body the single scroll container, so it shows the **browser's native
  scrollbar** rather than the custom styled scrollbar used elsewhere.
- Only `sticky: 'start'` is supported (no `'end'`).

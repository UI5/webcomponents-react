...
| Hook                                         | Registration Points    | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| -------------------------------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **useColumnDragAndDrop** (useDragAndDrop.ts) | `hooks.getHeaderProps` | Column reorder via drag-and-drop. Uses `dispatch` for `COLUMN_DND_START`/`COLUMN_DND_END` + `setColumnOrder`. No global disable knob — only per-column `disableDragAndDrop` is honored. Sticky columns are drop-**target**-excluded via `isStickyTarget = column.sticky === 'start'` (the **static** option, `useDragAndDrop.ts:17`), whereas the drag-**source** exclusion (`isDraggable`, `ColumnHeaderContainer.tsx`) uses the effective `stickyStartIndices` — so a runtime-only-pinned column is non-draggable yet can still be a drop target. |

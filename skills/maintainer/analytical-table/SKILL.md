...
Opt-in via `AnalyticalTableHooks.useStickyColumns()` in `tableHooks`. `@experimental`. Hook file: `pluginHooks/useStickyColumns.ts` (re-exported through the namespace).
...
- **Internal start columns** (`__ui5wcr__internal_selection_column`, `__ui5wcr__internal_highlight_column`) auto-pin **only when at least one user/grouped column is sticky** (`hasUserSticky` gate, `useStickyColumns.ts:80-85,126`). The navigation column (appended) never pins.
...
- **Auto-disable when too narrow.** In `useStickyMetadata`, sticky is dropped (`stickyStartIndices = []`) when `tableClientWidth - (minNonStickyColWidth + scrollbarWidth) <= totalStickyStartWidth`, where `minNonStickyColWidth` is `DEFAULT_COLUMN_WIDTH` (60px) on desktop / 88px mobile. Skipped on first render (`tableClientWidth === 0`); re-enables when the container grows (`useStickyColumns.ts:145-156`).
- **`onStickyColumnsChange(detail)` fires ONLY on the popover freeze/unfreeze**, never on programmatic toggles — the app already controls those calls. `detail = { column, sticky, stickyColumns }` (`AnalyticalTableStickyColumnsChangeDetail`, `useStickyColumns.ts:195-204`).
...
- **Drag subtlety:** the drag **source** is excluded via `isDraggable={... && !isStickyStart}` (state-based `stickyStartIndices`, `ColumnHeaderContainer.tsx:62`), while the drop **target** is rejected via `isStickyTarget = column?.sticky === 'start'` (the **static** option, `useDragAndDrop.ts:17`). A runtime-only-pinned column (no `sticky: 'start'`) is therefore non-draggable yet can still be a drop target.
